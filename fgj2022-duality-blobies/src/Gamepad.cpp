...
Gamepad::Gamepad(int number, bool XInput) : gamepadNumber(number)
{
	...
	// open db
	std::ifstream gamepads("assets/gamecontrollerdb.txt");
	if (!gamepads.is_open()) {
		printf("Gamepad database not found!");
		return;
	}
	...
	*/
	...
	if (!gamepads.eof()) {
		printf("'%s' found.", data.name.toAnsiString().c_str());
		LoadData(line);
	}
	else printf("Gamepad not found!"); // TO DO: Manually enter gamepad values
